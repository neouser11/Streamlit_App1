...
- Made interactive
- Set color to x value
- Give title to the chart
- Set filled of points to true
- Set size equal to y
""")

st.title('A scatter plot with 5 Changes')
scatter = alt.Chart(df).mark_point().encode(
    x='x',
    y='y',
    color='x',
...
- Added Title Bar Chart with Rolling mean
"""
)
st.title ('Bar Chart with Rolling mean')
source = data.wheat()
bar = alt.Chart(source).mark_bar().encode(
    x='year:O',
    y='wheat:Q'
)
line = alt.Chart(source).mark_line(color='yellow').transform_window(
